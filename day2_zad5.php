		...
		if(strlen($data_urodzenia)==10  && $data_urodzenia < date('Y-m-d') && $data_urodzenia > '1900-01-01'){	
			if(strlen($ulica)<50 && strlen($ulica)>3){
				if(strlen($miasto)<50 && strlen($miasto)>3){
							...
							$ins = mysqli_query($mysqli,"UPDATE clients SET id='$id', name = '$imie', surname = '$nazwisko', gender ='$plec', date_of_birth =  '$data_urodzenia', orders_count = 10, street = '$ulica', city= '$miasto', postcode='$kod_pocztowy', country=1, notes='' WHERE id='$id'");
